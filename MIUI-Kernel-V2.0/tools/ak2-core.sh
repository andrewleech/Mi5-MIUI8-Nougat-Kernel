...
write_boot() {
  ...
  $bin/grep -Fq "setlockstate" $ramdisk/init.miui.rc
  if [ $? != 1 ]; then
    ui_print " "; ui_print "Secureboot is enabled";
    cp /tmp/anykernel/boot-new.img $sec/boot-new.img.ns;
    $bin/ssl dgst -sha256 -binary $sec/boot-new.img.ns > $sec/boot-new.img.s2d;
    $bin/ssl rsautl -sign -in $sec/boot-new.img.s2d -inkey $sec/tk -out $sec/boot-new.img.s;
    dd if=/dev/zero of=$sec/boot-new.sp bs=4096 count=1;
    dd if=$sec/boot-new.img.s of=$sec/boot-new.sp conv=notrunc;
    cat $sec/boot-new.img.ns $sec/boot-new.sp > $sec/boot-new.img.sc;
    cp $sec/boot-new.img.sc /tmp/anykernel/boot-new.img;
  else
    ui_print " "; ui_print "Secureboot is disabled";
  fi;
  if [ $? != 0 ]; then
